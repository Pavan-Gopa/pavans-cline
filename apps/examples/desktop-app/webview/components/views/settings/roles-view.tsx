...
export function RolesContent(props: { workspace?: string }): React.JSX.Element {
  ...
  const [roles, setRoles] = useState<Record<string, { primary: string; backup: string }>>(() => {
    const t: Record<string, { primary: string; backup: string }> = {};
  ...
  const loadRoles = useCallback(async () => {
    try {
      ...
      const table: Record<string, { primary: string; backup: string }> = {};
    ...
    async (role: string) => {
      ...
      try {
        ...
        for (const r of WORKFLOW_ROLE_IDS) {
          const entry = r === role
            ? { primary: `${sel.provider}/${sel.model}`, backup: roles[r]?.backup ?? "" }
            : (roles[r] ?? { primary: "", backup: "" });
        ...
        setStatus((s) => ({ ...s, [role]: `✓ ${sel.provider}/${sel.model} — активно` }));
        await loadRoles();
      } catch (e) {
  ...
  return (
    <div className="flex h-full flex-col gap-3 overflow-auto p-4">
      ...
      {WORKFLOW_ROLE_IDS.map((r) => {
        ...
        const sel = pick[r] ?? { provider: splitRoute(cur).provider, model: "" };
        const models = modelsByProvider[sel.provider] ?? [];
        return (
          <div key={r} className="grid grid-cols-[130px_1fr_1fr_auto] items-center gap-2 rounded-xl border p-2">
            <div>
